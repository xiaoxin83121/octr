...
# TODO: if use global for sure, change to for to alleviate padding
def batch_dense(unique_coords, coords, features, foremask, mode):
    """
    args:
        unique_coords: coords for torch.unique, might be shifted
        coords: the coords of the sequence for computing local attention mask, [b_idx, z_idx, y_idx, x_idx]
        features: the features of the sequeces
        foremask: the foreground classification scores
        mode: opt in ['windows', 'global'], when 'global', using only b_idx for torch.unique
    return:
    ...
    if mode == 'global':
        unique_coords = unique_coords[:, 0]
    _, batch_inv, batch_counts = torch.unique(unique_coords, return_inverse=True, return_counts=True, dim=0)
    ...
    ret = torch.zeros((batch_size*max_n, features.shape[-1]), device=device)  # features
    ret_coords = torch.zeros((batch_size*max_n, 3), device=device, dtype=torch.int32)  # coords
    semantic_mask = torch.zeros(batch_size*max_n, device=device)  # foreground scores
    inds = torch.zeros(batch_size*max_n, device=device, dtype=torch.long)  # index of the sequence
    mask = torch.arange(max_n, device=device)[None, :] < batch_counts[:, None]  # padding mask
    # scatter
    ret[mask.view(-1), :] = features[sort]
    semantic_mask[mask.view(-1)] = foremask[sort]
    ret_coords[mask.view(-1), :] = coords[:, 1:][sort]
    inds.masked_scatter_(mask.view(-1), sort)
    
    # cal local mask and semantic mask
    # semantic mask
    semantic_mask = semantic_mask.view(batch_size, max_n)
    fusion_mask = 1 + repeat(semantic_mask, 'b n -> b m n', m=max_n)

    return ret.view(batch_size, max_n, features.shape[-1]), mask.int(), inds.view(-1, max_n), batch_inv, sort_inv, fusion_mask
...
class CrossAttention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.):
        ...
        self.to_q = nn.Linear(dim+4, inner_dim, bias=False)
        self.to_k = nn.Linear(dim+4, inner_dim, bias=False)
        self.to_v = nn.Linear(dim+4, inner_dim, bias=False)
    ...
    def forward(self, inp_q, inp_kv, attn_mask, topk, semantic_mask=None):
        """
        args:
        ...
        if semantic_mask is not None:
            semantic_mask = -10000. * (1- semantic_mask)
            dots += repeat(semantic_mask, 'b m n -> b h m n', h=h)
...
class VoxelClassifyHead(nn.Module):
    def __init__(self, in_channels, voxel_stride, voxel_size=[0.125, 0.05, 0.05], point_cloud_range=[-3, -40, 0, 1, 40, 70.4], indice_key='focal'):
        super().__init__()
        self.voxel_stride = voxel_stride
        self.voxel_size = torch.Tensor(voxel_size).cuda()
        self.point_cloud_range = torch.Tensor(point_cloud_range).cuda()
        self.inv_idx =  torch.Tensor([2, 1, 0]).long().cuda()
        self.conv_imp = spconv.SubMConv3d(in_channels, 1, kernel_size=3, stride=1, padding=1, bias=False, indice_key=indice_key+'_imp')
        self.focal_loss = FocalLoss()

    def forward(self, x, batch_dict):
        batch_size = x.batch_size
        spatial_indices = (x.indices[:, 1:] + 0.5) * self.voxel_stride
        voxels_3d = spatial_indices * self.voxel_size + self.point_cloud_range[:3]

        imps_3d = self.conv_imp(x).features
        ...
        return torch.cat((voxels_3d, mask_voxels.unsqueeze(-1)), dim=-1), loss_box_of_pts
...
class OctreeAttentionDense(nn.Module):
    def __init__(self, dim, nhead, head_dim, dropout, tree_depth, topks, layer_idx, norm_func=nn.BatchNorm1d, pos_emb=None, mode='global', wsize=5):
        ...
        self.norm_layer = nn.ModuleList([norm_func(dim+4) for _ in range(tree_depth)])
        self.proj = nn.Linear(dim*self.tree_depth, dim, bias=False)
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        self.local_pe = spconv.SparseSequential(
                post_act_block(dim+4, dim, 3, norm_fn=norm_fn, padding=1, indice_key=f'lpe_subm{layer_idx}'),
                post_act_block(dim, dim, 3, norm_fn=norm_fn, padding=1, indice_key=f'lpe_subm{layer_idx}'),
            )

    def process_coarse_level(self, feature, info_dict, topk, grid_size, foremask):
        """
        args:
            feature: [N d] (finest)
        ...
        coords = copy.deepcopy(kv_coords_compress)
        if self.mode == 'windows':
            coords[:, 1:] = torch.div(coords[:, 1:], self.wsize, rounding_mode='floor')
        ...
        foremask_this_layer = scatter(foremask[:, -1], kv_inv.long(), reduce='max', dim=0)
        ...
        dense_batch_features, dense_batch_mask, \
            dense_batch_inds, batch_inv_indices, \
            sort_inv, semantic_mask = batch_dense(coords, kv_coords_compress, query_feature, foremask_this_layer, mode=self.mode)
 
        out_feature, topk_area = self.top_layer(dense_batch_features, dense_batch_features, dense_batch_mask, topk, semantic_mask)
    ...
    def process_fine_level(self, feature, coord_dict, topk, prev_topk, kv_max_tokens, topk_area, topk_mask, parent_dict, idx, grid_size):
        ...
        # this_topk_inds = rearrange(this_topk_inds, '(n k) g -> n (g k)', k=prev_topk)[:, :kv_max_tokens]  # [N_i-1, 8k]
        # this_topk_inds_mask = rearrange(this_topk_inds_mask, '(n k) g -> n (g k)', k=prev_topk)[:, :kv_max_tokens]  # [N_i-1, 8k]
    ...
    def forward(self, data_dict):
        ...
        foremask = data_dict['foremask']
        voxel_features = torch.cat((foremask, voxel_features), dim=-1)
        ...
        out_feature, topk_area, topk_mask = self.process_coarse_level(voxel_features, data_dict['info_{}x'.format(2**(self.tree_depth-1))], 
                                                            ...
                                                            grid_size=data_dict['grid_size']//2**(self.tree_depth-1), foremask=foremask)
...
class OcTrV2_fore(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size,**kwargs):
        ...
        self.voxel_size = kwargs['voxel_size'][::-1]
        self.point_cloud_range = kwargs['point_cloud_range'].reshape(2, 3)[:, ::-1].reshape(-1)
        self.num_point_features = 256  # build detector_3d
        ...
        for i in range(self.block_nums):
            ...
            # self.ds.append(spconv.SparseConv3d(in_channel, self.dims[i], 3, stride=2, padding=1, bias=False))
        ...
        self.conv_out = spconv.SparseSequential(
            spconv.SparseConv3d(self.dims[-1], self.num_point_features, (4, 1, 1), stride=1, padding=0, bias=False),
            partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)(self.num_point_features),
            nn.ReLU(),
        )
        self.voxel_dis_head1 = VoxelClassifyHead(self.backbone_channels['x_conv3'], 4, self.voxel_size, self.point_cloud_range, 'dis1')
        self.voxel_dis_head2 = VoxelClassifyHead(self.backbone_channels['x_conv4'], 8, self.voxel_size, self.point_cloud_range, 'dis2')
    ...
    def forward(self, batch_dict):
        ...
        for i in range(self.block_nums):
            ...
            foremask, voxel_loss = getattr(self, f'voxel_dis_head{i+1}')(sp_tensor, batch_dict)
            loss += voxel_loss

            batch_dict['foremask'] = foremask
        ...
        self.forward_ret_dict['loss_box_of_pts'] = loss
        return batch_dict
